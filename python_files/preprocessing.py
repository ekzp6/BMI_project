...
annotated = pd.read_csv('C:/Users/ylime/Documents/FS20/BMI_intro/project/BMI_project/sample_annotation.txt',index_col=False,sep="\t")
samples1 = annotated[annotated["File"]==1]
samples2 = annotated[annotated["File"]==2]
samples3 = annotated[annotated["File"]==3]
samples4 = annotated[annotated["File"]==4]
print(samples1)

data = [raw_data1,raw_data2,raw_data3,raw_data4]
labels = [samples1,samples2,samples3,samples4]

#for i in range(len(data)):
#    data[i] = pd.merge(data[i],labels[i], how='left', on=['Block'])
